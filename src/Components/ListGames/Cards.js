...
export default function Cards({ games = [] }) {
  ...
  const handlePreviewClick = async (id) => {
    try {
      ...
      console.error("Gagal mengambil modal game:", error);
    } finally {
      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {games.map((game) => (
            <div
              key={game.id}
              className="bg-black border border-gray-700 hover:border-blue-500 transition-all duration-300 ease-in-out transform hover:scale-105 shadow-xl group rounded-lg overflow-hidden cursor-pointer flex flex-col"
            >
              <div
              ...
              >
                <Image
                  ...
                  height={190}
                  loading="lazy"
                  className="z-0 object-cover w-full transition-transform duration-500 group-hover:scale-110 rounded-t-lg"
                />
